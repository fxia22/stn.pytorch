  ...
  for(b=0; b < batchsize; b++)
  {
    for(yOut=0; yOut < output_height - 1; yOut++)
    {
      for(xOut=0; xOut < output_width - 1; xOut++)
      {
        ...
        basex[1] = (float)xOut / (float)(output_width-1) * 2 - 1;
        basex[2] = (float)(xOut+1) / (float)(output_width-1) * 2 - 1;
        basex[3] = (float)(xOut+1) / (float)(output_width-1) * 2 - 1;
         
        basey[0] = (float)yOut / (float)(output_height-1) * 2 - 1;
        basey[1] = (float)(yOut+1) / (float)(output_height-1) * 2 - 1;
        basey[2] = (float)yOut / (float)(output_height-1) * 2 - 1;
